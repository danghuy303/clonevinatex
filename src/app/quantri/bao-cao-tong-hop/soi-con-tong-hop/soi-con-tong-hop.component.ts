...
@Component({
  selector: 'app-soi-con-tong-hop',
  templateUrl: './soi-con-tong-hop.component.html',
  styleUrls: ['./soi-con-tong-hop.component.css']
})
export class SoiConTongHopComponent extends StoreBase implements OnInit {
  cols: any[];
  items: any[];
  sum: any = {};
  currentTime: any = {};
  ...
  GetBaoCao(){
    ...
    this._services.BaoCaoTongHop().GetBaoCao_SoiConTongHop(this.filter).subscribe((res:any)=>{
  ...
  exportBaoCao(){
    this._services.BaoCaoTongHop().XuatBaoCao_SoiConTongHop(this.filter).subscribe((res:any)=>{
        this._services.download(res.TenFile)
    })
  }
  guiMail(){
    this._services.BaoCaoTongHop().GuiEmail_SoiConTongHop(this.filter).subscribe((res:any)=>{
      if(res && res.State ===1){
