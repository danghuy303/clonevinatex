...
export class BaoCaoTongHopClassimatComponent extends StoreBase implements OnInit {
  ...
  getAllOptions() {
    forkJoin([this._services.GetOptions().GetMatHang(), this._services.DanhMucClassimat().GetList({ CurrentPage: 0 })]).subscribe((res: any) => {
      console.log(res);
  ...
  exportBaoCaoCa() {
    this.filter.Ngay = DateToUnix(this.filter.NgayChon);
    this._services.QuyTrinhClassimat().XuatBaoCao(this.filter).subscribe((res: any) => {
