...
@Component({
  selector: 'app-kiemkekhomuimodal',
  templateUrl: './kiemkekhomuimodal.component.html',
  styleUrls: ['./kiemkekhomuimodal.component.css']
})
export class KiemkekhomuimodalComponent implements OnInit, AfterViewInit, AfterViewChecked {
  ...
  ChuyenDuyet() {
    ...
    this.link.api().ChuyenTiep(this.item)
      .subscribe((res: any) => {
        if (res) {
          if (res.State === 1) {
  ...
  KhongDuyet() {
    this.item.listItem = deepCopy(this.listItem);
    this.link.api().KhongDuyet(this.item)
      .subscribe((res: any) => {
        if (res) {
          if (res.State === 1) {
            this.activeModal.close();
  ...
  XoaQuyTrinh() {
    ...
    modalRef.result
      .then((res) => {
        this.link.api().Delete(this.item)
          .subscribe((res: any) => {
            console.log(res);
            if (res?.State === 1) {
              this.activeModal.close();
