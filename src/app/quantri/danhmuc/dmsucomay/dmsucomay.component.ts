...
import { ModaldmsucomayComponent } from '../modal/modaldmsucomay/modaldmsucomay.component';

@Component({
  selector: 'app-dmsucomay',
  templateUrl: './dmsucomay.component.html',
  styleUrls: ['./dmsucomay.component.css']
})
export class DmsucomayComponent implements OnInit {
  ...
  GetListDanhMuc(reset?){
    ...
    this._services.DanhMucSuCoMay().GetList(data).subscribe((res:any)=>{
      this.items = res.items;
      this.paging = res.paging;
    })
  }
  add(){
    let modalRef = this._modal.open(ModaldmsucomayComponent,{
      backdrop:'static'
    });
    modalRef.componentInstance.opt='add';
    modalRef.componentInstance.title = 'Thêm mới danh mục sự cố máy';
  ...
  edit(item){
    ...
    modalRef.componentInstance.title = 'Cập nhật danh mục sự cố máy';
  ...
  delete(item){
    ...
    modalRef.result.then(res=>{
      this._services.DanhMucSuCoMay().Delete(item).subscribe((res: any) => {
        if (res) {
          if (res.State === 1) {
