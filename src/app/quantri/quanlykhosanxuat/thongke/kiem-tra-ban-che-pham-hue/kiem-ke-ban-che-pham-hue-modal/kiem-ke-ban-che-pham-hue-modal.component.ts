...
import { ChonmathangkiemtrabanchephammodalComponent } from '../../kiemtrabanchepham/chonmathangkiemtrabanchephammodal/chonmathangkiemtrabanchephammodal.component';
...
@Component({
  selector: 'app-kiem-ke-ban-che-pham-hue-modal',
  templateUrl: './kiem-ke-ban-che-pham-hue-modal.component.html',
  styleUrls: ['./kiem-ke-ban-che-pham-hue-modal.component.css']
})
export class KiemKeBanChePhamHueModalComponent implements OnInit {
  ...
  ChapNhan() {
    if (this.ValidateData()) {
      ...
      this._services.KiemKeBanChePhamHue().GetKhoiTaoPhieuKiemKeBanChePhamToHieu(data).subscribe((res: any) => {
        if (res.State === 1)
  ...
  KhongDuyet() {
    this._services.KiemKeBanChePhamHue().KhongDuyet(this.setData()).subscribe((res: any) => {
      if (res.State !== 1) {
        this.toastr.error(res.message);
      } else {
        this.toastr.success(res.message);
  ...
  XoaQuyTrinh() {
    ...
    modalRef.result
      .then((res) => {
        this._services.KiemKeBanChePhamHue().Delete(this.item).subscribe((res: any) => {
          if (res.State === 1) {
            this.toastr.success(res.message);
            this.activeModal.close();
          } else {
