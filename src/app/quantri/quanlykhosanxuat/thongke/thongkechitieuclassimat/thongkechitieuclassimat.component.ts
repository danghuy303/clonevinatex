...
@Component({
  selector: 'app-thongkechitieuclassimat',
  templateUrl: './thongkechitieuclassimat.component.html',
  styleUrls: ['./thongkechitieuclassimat.component.css']
})
export class ThongkechitieuclassimatComponent extends StoreBase implements OnInit {
  ...
  changeParam(id) {
    ...
    this.router.navigate([`/quantri/theodoithongkebaocaosanxuat/thongkechitieuclassimat/${id}`], { replaceUrl: true })
  }
  add() {
    this.changeParam(0);
    let modalRef = this._modal.open(ModalthongkechitieuclassimatComponent, {
      size: 'fullscreen',
      backdrop: 'static'
    })
  ...
  update(Id) {
    this._service.QuyTrinhClassimat().Get(Id).subscribe((res1: any) => {
      let modalRef = this._modal.open(ModalthongkechitieuclassimatComponent, {
        size: 'fullscreen',
        backdrop: 'static'
      })
  ...
  GetListQuyTrinh(reset?) {
    ...
    this._service.QuyTrinhClassimat().GetList(data).subscribe((res: any) => {
