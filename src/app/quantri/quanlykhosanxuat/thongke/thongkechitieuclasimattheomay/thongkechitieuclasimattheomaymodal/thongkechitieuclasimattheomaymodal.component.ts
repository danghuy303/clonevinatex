...
@Component({
  selector: 'app-thongkechitieuclasimattheomaymodal',
  templateUrl: './thongkechitieuclasimattheomaymodal.component.html',
  styleUrls: ['./thongkechitieuclasimattheomaymodal.component.css']
})
export class ThongkechitieuclasimattheomaymodalComponent implements OnInit {
  ...
  ChuyenDuyet() {
    ...
    this.services.QuyTrinhClassimatTheoMay().ChuyenTiep(this.item).subscribe((res: any) => {
      if (res) {
        if (res.State === 1) {
  ...
  KhongDuyet() {
    if (this.item.NgayKiemTra !== null && this.item.NgayKiemTra !== undefined)
      this.item.NgayKiemTraUnix = DateToUnix(this.item.NgayKiemTra);
    this.services.QuyTrinhClassimatTheoMay().KhongDuyet(this.item).subscribe((res: any) => {
      if (res) {
        if (res.State === 1) {
          this.activeModal.close();
          this.toastr.success(res.message);
  ...
  GetNextSoQuyTrinh() {
    this.services.QuyTrinhClassimatTheoMay().GetNextSo().subscribe((res: any) => {
  ...
  GhiLai() {
    if (this.item.NgayKiemTra === null || this.item.NgayKiemTra === undefined)
      ...
      this.item.NgayKiemTraUnix = DateToUnix(this.item.NgayKiemTra);
      this.services.QuyTrinhClassimatTheoMay().Set(this.item).subscribe((res: any) => {
        if (res) {
          if (res.State === 1) {
            this.toastr.success(res.message)
            this.opt = 'edit';
  ...
  XoaQuyTrinh() {
    ...
    modalRef.result.then(res => {
      this.services.QuyTrinhClassimatTheoMay().Delete(this.item.Id).subscribe((res: any) => {
        if (res?.State === 1) {
          this.toastr.success(res.message);
          this.activeModal.close();
        } else {
  ...
  GetMatHangTheoKho() {
    ...
    this.services.QuyTrinhClassimatTheoMay().GetListMatHang(data).subscribe((res1: any) => {
  ...
  ImportPhieuNhap() {
    ...
    this.services.QuyTrinhLoiCat().ImportPhieuNhapChiTieuClasimatTheoMay(this.TepImport.TenGui, obj.IddmPhanXuong, obj.NgayKiemTraUnix,this.item.IddmCaSanXuat).subscribe((imp: any) => {
  ...
  ExportPhieuNhap() {
    ...
    this.services.QuyTrinhLoiCat().ExportPhieuNhapChiTieuClasimatTheoMay(obj.IddmPhanXuong, obj.NgayKiemTraUnix,this.item.IddmCaSanXuat).subscribe((res: any) => {
