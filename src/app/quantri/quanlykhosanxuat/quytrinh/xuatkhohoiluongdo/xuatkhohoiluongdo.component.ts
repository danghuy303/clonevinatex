...
import { XuatkhohoiluongdomodalComponent } from '../xuatkhohoiluongdomodal/xuatkhohoiluongdomodal.component';

@Component({
  selector: 'app-xuatkhohoiluongdo',
  templateUrl: './xuatkhohoiluongdo.component.html',
  styleUrls: ['./xuatkhohoiluongdo.component.css']
})
export class XuatkhohoiluongdoComponent extends StoreBase implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this._service.GetListdmPhanXuongOpt().subscribe((res: any) => {
      ...
      let _this = res.find(x => x.IdDuAn === IdDuAn)
      this.listdmPhanXuong = mapArrayForDropDown(res, 'Ten', 'Id');
      if (_this) {
        this.filter.IddmPhanXuong = _this.Id
      }
    })
  }
  changeParam(id) {
    ...
    this.router.navigate([`quantri/quanlykhosanxuatbong/khohoiluongdo/xuatkho/${id}`], { replaceUrl: true })
  }
  add() {
    this.changeParam(0);
    let modalRef = this._modal.open(XuatkhohoiluongdomodalComponent, {
      size: 'fullscreen',
      backdrop: 'static'
    })
