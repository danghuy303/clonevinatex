...
@Component({
  ...
  templateUrl: './the-kho-gia-cong.component.html',
  styleUrls: ['./the-kho-gia-cong.component.css']
})
export class TheKhoGiaCongComponent extends StoreBase implements OnInit, OnDestroy {
  ...
  GetListQuyTrinh(reset?) {
    ...
    this._service.GetLuuKhoTheKho(data).subscribe((res: any) => {
      if (res) {
        ...
        this.paging = {
          ...res.paging,
          TotalItem: res.paging.TotalItem,
        }
      } else {
        this.items = [];
        this.paging = {};
