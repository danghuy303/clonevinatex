...
import { NhienlieumodalComponent } from './nhienlieumodal/nhienlieumodal.component';

@Component({
  selector: 'app-nhienlieu',
  templateUrl: './nhienlieu.component.html',
  styleUrls: ['./nhienlieu.component.css']
})
export class NhienlieuComponent implements OnInit {
  ...
  GetList(reset?: any) {
    ...
    this._danhMucTaiSan.LoaiNhienLieu().GetList(data).subscribe((res: any) => {
      this.items = res.Data.Items;
      this.paging.TotalCount = res.Data.TotalCount;
    })
  }
  add() {
    let modalRef = this._modal.open(NhienlieumodalComponent, {
      backdrop: 'static', size: 'lg',
    });
    modalRef.componentInstance.opt = 'add';
    modalRef.componentInstance.type = 'themmoi';
    modalRef.componentInstance.title = 'Thêm mới loại nhiên liệu';
  ...
  edit(item: any) {
    ...
    modalRef.componentInstance.title = 'Cập nhật loại nhiên liệu';
  ...
  delete(item: any) {
    ...
    modalRef.result.then(res => {
      this._danhMucTaiSan.LoaiNhienLieu().Delete([item.Id]).subscribe((res: any) => {
        if (res) {
  ...
  importExcel() {
    ...
    modalRef.result
      .then((res: any) => {
        console.log(res);
        this.fileUpload = res;
        this._danhMucTaiSan.LoaiNhienLieu().Importdm(this.fileUpload[0].Name).subscribe((res: any) => {
          if (res.StatusCode === 200) {
            this._toastr.success(res.Message);
            this.resetFilter();
          } else {
  ...
  exportExcel() {
    ...
    this._danhMucTaiSan.LoaiNhienLieu().Exportdm(data).subscribe((res: any) => {
      this._danhMucTaiSan.LoaiNhienLieu().download(res.Data);
    })
  }
