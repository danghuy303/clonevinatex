...
import { KiemDinhTaiSanPopupThemMoiComponent } from '../kiem-dinh-tai-san-popup-them-moi/kiem-dinh-tai-san-popup-them-moi.component';


@Component({
  selector: 'app-kiem-dinh-tai-san-modal',
  templateUrl: './kiem-dinh-tai-san-modal.component.html',
  styleUrls: ['./kiem-dinh-tai-san-modal.component.css']
})
export class KiemDinhTaiSanModalComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.item.NgayUnix !== 0 || this.item.NgayHetHanUnix !== 0 || this.item.NgayBatDauUnix !== 0) {
  ...
  GhiLai() {
    if (this.ValidateData()) {
      this._serviceTaiSan.KiemDinhTaiSan().Set(this.setData()).subscribe((res: any) => {
  ...
  GetNextSoQuyTrinh() {
    this._serviceTaiSan.KiemDinhTaiSan().GetNextSoQuyTrinh().subscribe((res: any) => {
  ...
  ThemMoiDanhSachTaiSan() {
    ...
    let modalRef = this._modal.open(KiemDinhTaiSanPopupThemMoiComponent, {
      size: "xl",
      backdrop: "static"
    });
  ...
  ChuyenDuyet() {
    this._serviceTaiSan.KiemDinhTaiSan().ChuyenTiep(this.setData()).subscribe((res: any) => {
  ...
  XoaQuyTrinh() {
    ...
    modalRef.result
      .then((res) => {
        this._serviceTaiSan.KiemDinhTaiSan().Delete(this.item.Id).subscribe((res: any) => {
          if (res.StatusCode === 200) {
            this.toastr.success(res.Message);
            this.activeModal.close();
          } else {
