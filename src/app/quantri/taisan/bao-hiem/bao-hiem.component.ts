...
export class BaoHiemComponent implements OnInit, OnDestroy {
  $sub!: Subscription;
  ...
  GetList(reset?) {
    ...
    this._serviceTaiSan.QuyTrinhBaoHiem().GetList(data).subscribe((res: any) => {
  ...
  changeParam(id) {
    this.router.navigate([`quantri/taisan/bao-hiem/${id}`], {
      replaceUrl: true,
    });
  }
  add() {
    let modalRef = this._modal.open(BaoHiemModalComponent, {
      backdrop: 'static',
      size: 'fullscreen-100',
      keyboard: false
    });
